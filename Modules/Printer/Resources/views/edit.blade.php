                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 style="font-family: 'Cairo', sans-serif;" class="box-title">{{ 'تعديل مطبعة' }}</h3>
                        </div>
                    @include('common::partials._errors')
                    <!-- /.box-header -->
                        <div class="box-body">
                            <form action="{{ route('printers.update',$row->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                    ...
                                    @if(auth()->user()->role_id == 1 || auth()->user()->role_id == 2)
                                        <select class="form-control" name="user_id">
                                            @foreach($users as $user)
                                                <option {{ $row->user_id == $user->id ? 'selected' : '' }} value="{{ $user->id }}">{{ $user->name }}</option>
                                            @endforeach
                                        </select>
                                    @else
                                ...
                                <div class="form-group">
                                    ...
                                    <img style="width: 70px;height: 70px;" id="blah" src="{{ url('storage/printers/'.$row->logo) }}" alt="your image" />
